...
async function main(): Promise<void> {
  ...
  const worker = new StagingWorker(env.RABBITMQ_URL, job);

  const shutdown = async (signal: string) => {
    console.log(`[run-staging-worker] ${signal} — shutting down`);
  ...
  console.log('[run-staging-worker] starting...');
  await worker.start();
}

main().catch((err) => {
  console.error('[run-staging-worker] fatal:', err);
  process.exit(1);
});
