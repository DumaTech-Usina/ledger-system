...
func main() {
	...
	if err != nil {
		log.Fatal("[proposal-batch-consumer] failed to connect:", err)
	}

	if err := infraMongo.EnsureIndexes(ctx, conns.MongoDB); err != nil {
		log.Fatal("[proposal-batch-consumer] failed to ensure indexes:", err)
	}
	...
	handler := jobs.NewProposalBatchHandler(proposalRepo, receiptRepo, auditRepo, eng)

	worker := rabbitmq.NewWorker(amqpURL, rabbitmq.WorkerConfig{
		Queue:      "validators.proposals.batch",
		RoutingKey: "validation.proposals.batch",
		DLQueue:    "validators.proposals.batch.dead",
	}, handler)

	log.Println("[proposal-batch-consumer] starting — waiting for blocking-key batches")

	if err := worker.Start(ctx); err != nil && !errors.Is(err, context.Canceled) {
		log.Fatal("[proposal-batch-consumer] fatal:", err)
	}

	log.Println("[proposal-batch-consumer] shutting down")
}
