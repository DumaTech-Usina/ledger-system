 ...
 * become `errors`/`warnings` on the returned result — the caller always gets back exactly what
 * happened, including whichever fields DID extract successfully.
 */
export class DocumentExtractionService implements FileExtractionPort {
  ...
  async extract(file: RawDocument): Promise<ExtractionResult> {
    // Identify format / select adapter.
    ...
    // Validate found data — demote anything implausible to a warning instead of passing a
